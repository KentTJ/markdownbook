...
所以，<font color='red'>Handler本质与主线程 没关系、与UI没关系</font>
...
>
> 3、<font color='red'>所有的一切都是主线程的</font>（or Looper线程），是一个输入系统。<font color='red'>调用线程没有任何东西。</font>
...
>   1、马路上一个个 汽车（msg ），一个个排队 走。这个时候，xin 的车(异步msg ) 要来了，
>
>   屏障msg是 警察。<font color='red'>警察插入队伍中某个位置（when），封住后面所有的车</font>  。  
>
>   ![image-20260302013757848](线程间通信.assets/image-20260302013757848.png)
>
>   2、**等待总统车的到来（可能比较晚16ms后）**
>
>   3、总统过后，移除栅栏
...
-<font color='red'>本质：</font>屏障消息就是为了确保异步消息的优先级，~~即：设置了屏障后，只能处理其后的异步消息，同步消息会被挡住~~
...
>   -<font color='red'>为 总统的车（VSync消息？？？）开路</font>： VSync 16ms一次，如果用户同步消息（<font color='red'>多个，很可能</font>）没有
>
>   执行完成，无法执行VSync消息，无法画用户界面。在用户角度，出现了掉帧或卡顿的情况。
...
> ViewRootImpl接收屏幕垂直同步信息事件用于驱动UI测绘----------------view.invalidate触发重绘
>
...
```java
// requestLayout()或invalidate()时---->
// ViewRootImpl.java
void scheduleTraversals() { // 计划
    if (!mTraversalScheduled) {
        mTraversalScheduled = true;
        
        // 【第一步】：立刻向 MessageQueue 插入同步屏障！
        mTraversalBarrier = mHandler.getLooper().getQueue().postSyncBarrier();
        
        // 【第二步】：向 Choreographer 注册，等待下一次 VSYNC 信号
...
解决单通道 FIFO 的“队头阻塞” (HOL Blocking)

###  认知塌缩到 0：将其理解为“用户态的中断屏蔽”

-**<font color='red'>同步屏障，就是 UI 线程的“用户态中断（Interrupt）”机制。</font>font>**

如果你熟悉操作系统底层的中断机制，你会发现它们在设计哲学上一模一样：

-   **常规执行：** CPU 逐行执行普通的应用程序代码（相当于 Looper 执行普通同步 Message）。
-   **硬件 VSYNC 信号到来：** 相当于网卡或硬盘给 CPU 发出了一个硬件中断（Hard IRQ）。
-   **立起同步屏障 (`postSyncBarrier`)：** 相当于 CPU 接收到高优先级中断后，立刻执行汇编指令 `cli`（Clear Interrupts，**关闭/屏蔽所有普通中断**）。此时，普通程序的执行被瞬间冻结。
-   **执行异步消息 (`doFrame`)：** 相当于 CPU 跳转去执行 **中断处理程序（ISR - Interrupt Service Routine）**。
-   **移除屏障 (`removeSyncBarrier`)：** 相当于中断处理完毕，CPU 执行 `sti`（Set Interrupts，**恢复中断**），普通程序继续从断点往下跑。

**塌缩结论：** 不要把同步屏障当成队列里的一条消息，把它当成一个**开关**。开关按下，<font color='red'>世界静止，只有 VSYNC 这个“神”在行走；</font>开关弹起，世界恢复喧嚣。
