...
-------> 使用width=250去触发，方便修改width、方便调整层级
...
-<font color='red'>软件 = 逻辑 + 配置</font>

最优的状态：

> 代码里只有逻辑（即<font color='red'>所有配置来源于文件</font>、或者外部接口）

-----------> 目的： 可以做到灵活配置，不用重新编译：

> 1、对于调试很优
>
> 2、对于代码维护很优：代码量太大（不能大海捞真修改配置）
>
> 3、对于测试很优：有问题，测试自己可以搞定

例子：

> 在destopshell.c里修改 weston背景透明度、在compositor里修改透明度
>
> -----------------> 都不如：在ini配置里修改

再例子：

> log的debug开关，应该在文件里，<font color='red'>最好做到实时（不用重启）</font>
...
谁来承载不同？
**不同分支、不同dst、不同的config文件**



## 动态开关-------实时文件

在运行态可以改变，不用重启 -----------> 可以保留问题现场



动态开关：

> 1、动态开关是一种化简：<font color='red'>让保留尽可能多的log，成为可能</font>
>
> 2、如果**没有动态开关**，以前的加的log，过于繁重  --------> 一定不会保留！！！！**造成重复劳动，没有积累**
