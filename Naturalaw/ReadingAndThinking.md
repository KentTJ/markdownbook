...
> 第一次是：总结  <font color='red'>技巧</font>
>
> 第二次是：总结  <font color='red'>结构，化简</font>
...
有了结构，功能是自然的事情。（比如，`代码所有类都new出来了，之间的相互功能，以及时序都是自然的`。。。。再比如，排兵布阵好了，怎

么工作也是自然的。。。再比如，车能跑，因为圆形轮子结构）物体如此，`代码如此`，社会组织更是如此。
...
>   <font color='red'>树结构 = 即树根（0层逻辑） + 树干（1层逻辑） + 树枝（2层逻辑） +  树叶（N层逻辑）</font>
>
>   <font color='red'>树根是本质</font>
>
>   <font color='red'>没有真正理解树根，就不会理解树叶</font>
>
>   <font color='red'>树给出了主次：</font>关注次要，忽略主要 -----> 极其致命
>
>   >   投资树。。。只关心投资的0层，1层逻辑。。。看不懂0层和1层，不用关心细
>
>   人的思维一定是线性的（做不到其他复杂结构）。所以，**做到线性的工具，不停问自己主次**

执行层面，规定：

>   1、任何一件事情（超大、超小），<font color='red'>都要给出树结构（以图的形式）</font>。。。比如：窗口知识点的树结构、分屏的树结构、股票的树结构
>
>   2、形式上做化简：必须划去树结构的  ~~树叶和树枝~~ 
...
> 1、代码架构/机制①  与 其他  代码架构/机制②  同构
> 
> 2、代码架构/机制①  与  生活中例子 同构
...
我们不可能理解、也不可能记忆、也不可能搜索  树状结构（甚至链表结构）

------>  我们要做的事情，就是高度压缩（塌缩）成一个点（约束）

例子：

> -<font color='red'>极好的压缩利器----- sh的函数记录笔记</font>：（N2复杂度 -----> 1。<font color='red'>绝对扁平化 ------ 没有记忆负担  &  搜索负担</font>）
> 
> 
> ```
> 查找PSS时：------------> 复杂度1： mem_he_ | grep PSS
>  
> function mem_he_()
> {
>    echo '''========================  =============================
...
>   网红 和 宗教领袖、主席都是一样的
>
>   ---------------> <font color='red'>关键是链接 & 团结多少人</font>， 链接结构，这是价值
...
>   股市有时间性
>
>   李嘉诚第一个投入塑胶花生产、3000家以后就放弃塑胶花生产  --------> 时间性
>
>   67年抄底房地产，17年 7折抛售房地产 --------> 时间性

-----------------> 从时间性的角度来看，李做投资，也会非常成功





时间性拆解  -------->  了解世界最新消息：英文、报纸
...
>   人要慎独（人的状态会处于混乱，如果没有约束的话）
>
>   不要相信自己的自律

## 数学 -----------基于概率思考 & 检测每个念头

-<font color='red'>不要相信  机械式 因果</font>！！！！

原因在于：<font color='red'>生活太过于复杂，我们没法知道所有变量 & 各个变量的比重 & 时间的变化</font>
...
>   >   ​                      博闻强识，却没有自己化简后的东西
>
>   记忆的奴隶：需要记忆很多东西 ---------> 电脑存储 + 快速搜索
>
>   念头的奴隶：一个念头反复咀嚼，比如考试前的紧张  ----------> <font color='red'>脑子反复空转</font>
...
-<font color='red'>**如果这世上只有一件事情重要，那便是化简**！！！！</font>（简----> 繁，学校、社会、项目会逼着我们做）



**<font color='red'>站在任意的一个点上，一定要做两件事情：</font>**
...
>   <font color='red'>向下拿到本质和精髓，才有可能向上  通用迁移</font>
...
2、由于知识是已经完成的事物。自己去看，必然是 从上往下；我们很难理解（甚至看不到）从下往上（房子最开始有没有搭地基、房子

内部结构是怎么设计的）   ------------->    如何更好地逆向？如何从上到下的角度更好地理解？
...
**抛开代码，抛开具体，，，，，，其 背后的模型（结构）是什么？？？？**

-<font color='red'>证明</font>，即:

>   专家不记忆代码，专家理解约束（Constraints）---------> **代码只是为了满足“约束”而不得不写出的结果**。
>
>   即：<font color='red'> 理解约束 + 输入 + 输出</font>   -----------><font color='red'> 做证明（证明不得不！！！！）</font>



闭眼！！！！！！！！！

分析框架之<font color='red'> 5W2H + s(structure) + i(import, 什么是重要的)</font>
...
-<font color='red'>追求利，而不是追求真</font>（真意味着现实、意味着悲观！！！！意味着喋喋不休）  -------> 学习李嘉诚！！
...
**任何系统的设计，最终都逃不过三个“物理定律”的约束**：

1.  **时序依赖（Causality）：** B 依赖 A 的数据，那么 A 必须先发生。
2.  **资源边界（Isolation）：** 内存是不共享的，进程是隔离的，硬件是独立的。
3.  **物理瓶颈（Bottleneck）：** 带宽有限，延迟存在，刷新率固定。

**不要背诵“是什么”（What），要去推导“为什么必须是这样”（Why it has to be）。**

当你遇到一个新的系统（比如你正在研究的虚拟化显示），试着问自己：
...
```java
心法一：信息流的“时空守恒律”
核心逻辑： 如果我要完成任务 C，我必须先知道 B；要算 B，必须先知道 A。API 的调用顺序，本质上是数据的依赖顺序。

例子：为什么 View 的绘制流程必然是 onMeasure -> onLayout -> onDraw？
很多初学者死记硬背这个顺序，但如果你用“必然性”去推导：

目标： 我要把像素画在屏幕上（onDraw）。

约束： 画在哪？多大？

如果我不知道自己多大，我就没法画（比如一个居中的文字，不知道字多宽，怎么算居中坐标？）。 -> 所以 onMeasure 必然在 onDraw 之前。

如果我不知道父容器给我的位置（左上角坐标），我算出大小也没用，因为我不知道像素填在显存的哪个 offset。 -> 所以 onLayout 必然在 onDraw 之前。

冲突： 是先算大小（Measure）还是先算位置（Layout）？

如果不先算好每个子 View 的大小，父容器根本无法决定怎么摆放它们（比如 LinearLayout 要把子 View 挨个排开，不知道子 View 宽度怎么排？）。

结论： 必然是 Measure（自下而上算大小） -> Layout（自上而下定位置） -> Draw（最后填像素）。

如何应用到你的 DMA 项目？
如果你在设计 Guest 到 Host 的传输：

必然需要先协商内存布局（Descriptor） -> 才能填数据 -> 才能触发中断通知 Host。顺序不可逆，API 必须照此设计。

心法二：信任边界的“交互协议律”
核心逻辑： 当两个模块（App vs System，Guest vs Host，CPU vs GPU）内存不共享、互不信任时，必然需要一个“中介”和一种“打包格式”。

例子：为什么 Binder（或任何 IPC）的 API 长成那样？
为什么会有 Parcel？为什么要有 transact 和 onTransact？

物理约束： 进程 A 的指针 0x1234 在进程 B 里是无效的（野指针）。

必然推导：

既然指针不能传，那我也不能直接传对象引用。

所以我必然要把对象里的数据（int, string）抠出来，按顺序塞进一段连续内存里（序列化）。 -> Parcel 类及其 API (writeInt, readInt) 诞生了。

既然是一段连续内存，接收方怎么知道这段内存里第一个是 int 还是 string？

结论： 发送方和接收方必然要约定好读取顺序（协议/IDL）。

如何应用到你的 Wayland/Display？

Wayland 的 Client 和 Compositor 在不同进程。

Client 想画图，不能直接给 Compositor 一个 void* 指针。

必然性： 必须通过 mmap 共享内存（Shared Memory）或者 DMA-BUF handle（文件描述符）。

API 预测： 所以 Wayland 协议里必然有传递 fd (file descriptor) 的接口，且必然有 wg_surface_attach 这种把 buffer 绑定的动作。不需要背，想通了跨进程内存限制，它只能这么干。

心法三：硬件物理的“节拍同步律”
核心逻辑： 软件必须服从硬件的物理特性。屏幕刷新率是固定的（60Hz/120Hz），这根“指挥棒”决定了所有 API 的时机。

例子：为什么会有 Choreographer 和 VSync？
早期的 Android 随便什么时候都能 requestLayout，为什么后来变了？

物理约束： 屏幕每 16.6ms 扫完一帧。如果你在第 15ms 还在画图，这一帧就赶不上了（撕裂或掉帧）。

必然推导：

软件不能想画就画，必须看着硬件的脸色行事。

结论： 系统必然需要一个信号（VSync）来告诉 App：“现在开始画，你有 16ms 时间”。

API 预测： App 层必然有一个回调机制（doFrame），只有收到这个信号才能开始 measure/layout/draw。这就是 Choreographer 存在的物理必然性。
```
...
<font color='red'>不停地问：关键的是什么？   价值是什么（值得做吗）？   能做到复利吗（护城河、不变性）？  </font>
...
https://www.youtube.com/watch?v=Cj2O90rpobE  老登公关为什么总是让人匪夷所思，谈谈年轻人破局的新机会
...
-<font color='red'>识别美好背后的结构，然后做迁移</font>
...
美好之抓<font color='red'>某一特征下</font>的trace：

>   问题：trace只能抓10s，因为很大（trace的本质：dump一段时间）
>
>   **结构**：<font color='red'>dump一段时间</font>  + <font color='red'> 特征值触发问题</font>
>
>   ------------> 解决这一结构的统一方法：循环执行 + 特征值出现后暂停

## 对于割裂的东西 尝试去联系；对于 联系的东西  尝试去 割裂：-------->  <font color='red'>思想</font>

----------> 例子： 多屏开机动画联动

## 荣耀任意门： -------->  背后思想：（1）、**减少路径**（复制、下载的） （2）、打破App之间的壁垒
