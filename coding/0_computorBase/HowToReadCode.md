...
- **结果（代码形态的<font color='red'>不得不</font>）：** 代码里**绝对不能**有频繁的 `memcpy`（内存拷贝）。如果有，系统必挂。
...
- **约束：** Android 必须要有多窗口叠加。
    
- **推导 <font color='red'>不得不</font>：** 必须有一个合成器（Composer）。
    
- **结果（代码形态）：** 代码里**必然**有一个 `Z-Order` 排序逻辑，且一定有处理 `Transparent Region`（透明区域）的优化算法。
...
> 我们能轻易记住 功能（大功能、小功能），但是记不住  对应的 大函数、小函数
...
-<font color='red'>规定：先列出主要功能点</font>
...
>   为啥架构是这样的？可不可以是其他样子？
>
>   <font color='red'>证明 = 说清楚，架构的0层逻辑是什么？1层逻辑是什么？</font>
