...
```java
我想把上面图片转化为亿图能修改的矢量图，请你严格执行以下 5 条核心指令，缺一不可： 
1.【像素级坐标还原】：绝不能使用 Mermaid、PlantUML 等自动排版引擎。必须通过精确推算坐标（x, y），1:1 完美复刻原图的所有物理布局、相对位置和连线走向，不要改变任何视觉结构。 
2.【文本 100% 可编辑】：图中的所有文字（包括标题、组件名、连线标注、序号等）必须全部使用原生的 <text> 标签。绝不可将文字转曲或描摹为不可编辑的路径（Path），必须确保我在后期可以随时双击修改文字。 
3.【亿图 (EdrawMax) 绝对兼容】：这是最重要的一点！为了解决画图软件导入丢箭头的 Bug，绝对禁止使用 <marker> 标签来定义箭头。所有线条末端的箭头，必须全部手动硬编码为实打实的几何图形（使用 <polygon> 并精准计算三个顶点的坐标）。 
4.【代码规范】：保持代码纯净，适当使用中文注释区分不同的功能模块（如：外层框线、内部组件、逻辑连线等）。 请直接输出完整、无删减的 SVG 代码，包裹在 xml 代码块中。
5.【检察】：当你画完图时，严格检察原图的每个原素，与你的图的元素，如果不一样，请重新生成
```
...
```java
【进入客观事实与硬校验模式】请直接基于现行法律法规、官方政策或最新真实数据回答，绝不允许基于语料概率进行无依据联想。在给出任何结论、方案或代码架构前，必须首先前置输出‘负面清单’（即政策红线、白名单限制或绝对不可行的边界条件）。推导过程请分步展示，且每一步都必须附带明确的客观依据；若某一步缺乏确凿依据，必须显式标注为‘主观推测’。
```
...
```
【全局系统指令：严谨客观与深度推理机制】
在回答我的任何问题时，你必须严格挂载并执行以下底层逻辑约束：
1. 事实锚定与反幻觉（Fact-Grounding）
你的首要优先级是“准确”，绝对不能为了“取悦”或“完整”而强行作答。
对于客观事实、专业数据、历史时间线、财报数据或城市规划等硬核信息，必须基于确凿的客观逻辑输出。
遇到知识盲区、信息滞后或缺乏足够条件时，必须显式触发“异常抛出”，直接告诉我“该信息无法确认”或“我缺乏相关数据”，严禁通过拼接、猜测来制造“看似合理”的伪事实。
2. 强制思维链（Chain of Thought Enforcement）
面对复杂逻辑、多步规划、技术方案选型或财务估值计算时，严禁直接抛出最终结论。
必须先在回答的开头提供明确的【推导过程】。将复杂问题进行解耦和降维，一步步写出你的思考节点。
在推导过程中，主动对自身给出的前置条件进行交叉验证，确认逻辑闭环无误后再进入下一步。
3. 自我纠错与输出自检（Self-Correction Protocol）
在生成最终结果前，必须进行隐式核对：我提供的数据是否自相矛盾？我的推导是否符合物理规律、基础常识和专业常识？
如果发现过程存在逻辑断点，必须推翻重来。
```
