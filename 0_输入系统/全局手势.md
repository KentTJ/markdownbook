...
![image-20250519002704863](全局手势.assets/image-20250519002704863.png)
...
aosp中 SystemGesturesPointerEventListener 与 InputMonitor 的区别（个人理解）：

>   系统级的手势（比如返回手势、任务切换手势等）
>
>   与 UI 操作无关的手势，比如滑动、缩放等
...
<img src="全局手势.assets/AndroidGesture.svg" alt="AndroidGesture" style="zoom:67%;" />

[图来源](https://juejin.cn/post/7209964630160834615)  & AndroidGesture.eddx

注意：<font color='red'>事件是并行分发：</font>

>   一个motionevent 同时 给InputMonitors & touchedwindow
>
>   并行分发<font color='red'>的补救：</font>cancel事件
...
在dispatchEventLocked()内部会遍历inputTargets进行发送，也就是说，当我们在触摸屏幕后事件在派发时，**不仅会派发到当前focus窗口，也会发送到全局手势监听者，**

产品侧的证明：屏幕左侧滑动退出应用，如果以模拟发送back键来实现时，在退出应用前，会看到应用内部也会响应左滑

代码证明：

```java
// 【】 将Monitor的channel也放到分发inputTargets
...
### 注入cancel事件（判定为手势了，mInputMonitor.pilferPointers通知其他monitors和window）

核心思想：

>   ![Image](全局手势.assets/640)
>
>   图来源: [漏学Input知识系列之“偷”走了其他窗口的事件pilferPointers](https://mp.weixin.qq.com/s/l_mQK1xs8d4dp7VMkl0kyw)
...
>   1、<font color='red'>从图中，功能角度必然要做的三件事情：规则123</font>
>
>   2、具体代码实现：参考：https://mp.weixin.qq.com/s/l_mQK1xs8d4dp7VMkl0kyw 
>
>   ```java
...
>           //【】 规则：1、对其他的Monitor & TouchedWindow 发送Cancel事件 2、对当前monitor不发cancel（因为当前摸monitor是触发者，后续可能还要事件）
>           if (channel != nullptr && channel->getConnectionToken() != token) {
>               synthesizeCancelationEventsForInputChannelLocked(channel, options); // 【】发送cancel
>               ..............
>           }
>       }
>   
>       state.cancelPointersForWindowsExcept(window.pointerIds, token); 
>         --> for循环，把state.windows列表里的清理掉：
>              w.pointerIds &= ~pointerIds; 
>             clearWindowsWithoutPointers()  // 【】规则3：必然要拦截其他窗口事件 = (具体：窗口列表state.windows中移除非token的window，后续move和up只会发token对应的window---即触发的monitor)
>   }
>   ```
>
>   
...
>   ~~补充：规则3~~
>
>   >   <font color='red'>move事件的分发逻辑就是 对 窗口列表state.windows （down确定下来的）进行分发</font>
>   >
>   >   见：《ACTION MOVE的分发》
...
```java
05-26 16:18:57.958   622  2474 I InputDispatcher: Channel [Gesture Monitor] edge-swipe (server) is stealing touch from [a7c19a3 com.android.dialer/com.android.dialer.main.impl.MainActivity (server), [Gesture Monitor] swipe-up (server)]
05-26 16:18:57.958  1303  1303 D TaplEvents: TIS / TouchInteractionService.onInputEvent: MotionEvent { action=ACTION_CANCEL, actionButton=0, id[0]=0, x[0]=54.975586, y[0]=1103.9443, toolType[0]=TOOL_TYPE_FINGER, buttonState=0, classification=NONE, metaState=0, flags=0x20, edgeFlags=0x0, pointerCount=1, historySize=0, eventTime=11985675, downTime=11985575, deviceId=4, source=0xd002, displayId=0, eventId=1567873396 }
05-26 16:18:58.265  3301  3309 W ndroid.systemui: Cleared Reference was only reachable from finalizer (only reported once)
```



### framework层拦截了kernel的UP事件（避免给到应用）
...
1、up与cancel都是Down的事件完备。**但是区别巨大:** 不能up替代cancle，原因:click事件依靠up

2、**时间尺度：**

>   事件报点是ms级（10ms），而上层手势是s级（如果愿意，也可以达到min级别）
