...
>   （1）画架构图（<font color='red'>成为你的地图</font>）：系统整体架构、详细组件架构、编译构建架构、线程关系架构图、时序图（运行时架构流程）
>
>   ​        AI将代码转化为图像  -------> 你去理解图像
>   
...
> [!NOTE]
> **【Mermaid 绘图强制规则：防截断 + 自动换行】**
> 
> 1. **强制换行**：对于超过 3 个单词或 10 个字符的节点文本，**必须使用 `<br/>` 进行手动换行**，确保单个节点宽度适中。
>     
> 2. **严格引用**：所有文本必须包裹在双引号 `""` 中（例如 `id["第一行<br/>第二行"]`），否则换行标签不生效。
>     
> 3. **布局方向**：默认使用 `graph TD`（从上到下），严禁使用 `LR`（从左到右），以适应窄屏阅读。
>     
> 4. **特殊符号**：严禁在 ID 中使用特殊字符，只允许在双引号内的 Label 中使用。
