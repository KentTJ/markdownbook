...
计算：
1、1-7月 base: 3.36*1.36 = 4.57              -------->   -- 32w
   8-12月 base：3.76 * 1.36 = 5.11           -------->   -- 25.55
...
季度奖：
                        纳税App    真实月份
                         2月       1月（和流水❌️7.1=4.57 + 2.53） ----------> 2.53似乎是加班费？？？？
                         3月       2月（和流水✅️4.6 = 4.57）：
                         4月       3月（和流水✅️4.6 = 4.57）：
                         5月       4月（和流水✅️6.6 = 4.57 + 2.03）0.6*3.36 季度奖
                         6月       5月（和流水✅️7.5 = 4.57 + 2.9）有火车头激励0.6(2.01w) -----> 还有0.9来源于哪里？？？？
						 
                         7月       6月：（和流水 7.4 = 4.57 + 2.83）2.83 = 0.84*3.36  绩效部分 
                              
                         真实工资增加月份！！！-----> 7月
                         8月：       7月：（和流水✅️ 5.79 = 4.57 + 1.22）0.18*3.76 0.18是6月多余的 ----------> 不满足标准工资部分？？？？
                         9月：       8月：（和流水✅️5.15）
                         10月：      9月：（和流水✅️6.5 = 5.11 + 1.31）0.35 * 3.76 = 1.31                          2025-09 系数0.71（0.36+0.35）               -----》 0.35与奖励明细里0.28 差了 0.07
                         11月       10月：（和流水✅️5.18）
                         12月       11月：（和流水✅️5.14）
                         26/1       12月：（和流水❌️ 8.0 = 5.11+ 1.88 + 1一等功）：0.5 * 3.76 = 1.88                2025-12 系数0.86（0.36+0.5）      -----》 0.5与奖励明细里0.408 差了 0.09
...
差额14.3 = 5月火车头2.03w
           + 1月加班 2.53
           + ️1w 一等功 
           + 差额8.74       8.74差额 = 四月季度奖2.03 + 5月（不明0.9w？？）+ 6月季度（2.83）+ 7月份1.22 虚拟项目奖 + 9月季度1.31  + 12季度1.88
虚拟：
差额14.3= 季度奖8.05 + 项目奖4w（1月加班费2.53+ 5月火车头2w）+ 一等功1w 
			  1月项目奖2w、5月项目奖2.9w 7月份项目奖1.2w
...
![mermaid-diagram-2026-01-26-191206](无障碍_.assets/mermaid-diagram-2026-01-26-191206.png)

```
sequenceDiagram
    autonumber
    participant TE as TouchExplorer (System)
    participant AMS as AccessibilityManagerService (System)
    participant AIF as AccessibilityInputFilter (System)
    participant AS as TalkBackService (Process)
    participant VRI as ViewRootImpl (App Process)
    participant View as View (App Process)

    Note over TE, View: 阶段一：手势采集与分发 (ACTION_MOVE -> GESTURE_DETECTION)

    Input ->> TE: onMotionEvent (下滑/右滑操作)
    TE ->> TE: handleMotionEventStateTouchExploring
    TE ->> AMS: sendAccessibilityEvent (TYPE_GESTURE_DETECTION_START)
    AMS ->> AIF: notifyAccessibilityEvent
    AIF ->> AS: onAccessibilityEvent (TalkBack 接收到手势开始信号)

    Note over TE, View: 阶段二：手势识别与指令下发 (AMS -> AS -> AMS)

    TE ->> AMS: onGestureCompleted (识别为 GESTURE_SWIPE_RIGHT)
    AMS ->> AS: onGesture (通知 TalkBack 手势识别完成)
    Note right of AS: TalkBack 决定下一个获焦节点
    AS ->> AMS: performAction (ACTION_ACCESSIBILITY_FOCUS)

    Note over TE, View: 阶段三：跨进程执行与 UI 反馈 (AMS -> APP)

    AMS ->> VRI: performAccessibilityAction (通过 IAccessibilityInteractionConnection)
    VRI ->> VRI: handleMessage (MSG_PERFORM_ACCESSIBILITY_ACTION)
    VRI ->> View: findViewByAccessibilityId
    View ->> View: performAccessibilityActionInternal

    %% 优化后的矩形框，设置 rgba 透明背景，避免白色遮挡
    rect rgba(0, 150, 255, 0.3)
        Note right of View: 处理 ACTION_ACCESSIBILITY_FOCUS
        View ->> View: requestAccessibilityFocus
        View ->> AMS: sendAccessibilityEvent (TYPE_VIEW_ACCESSIBILITY_FOCUSED)
        View ->> VRI: setAccessibilityFocus(this)
        View ->> View: drawAccessibilityFocusedDrawable (绘制绿框)
    end
```
