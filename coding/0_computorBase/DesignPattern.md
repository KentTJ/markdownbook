...
## why--------为什么需要回调？本质解决什么问题？



when: 不清楚正向调用完成的时间点时，需要回调通知（如果知道，就不需要了）

例子：App通知系统去hide窗口：

> 情况一：如果系统侧在一帧内，立即hide完，则App侧代码：
>
> ```java
> // 代码
> window.hide()
> 执行hide之后的操作--------> // 这一步之前，保证了系统侧的window已经hide了
>                                    或者   系统是否hide窗口  不影响这一步
> ```
>
> 情况二：系统侧不能立即执行完hide/精确时间不定
>
> ```java
> // 代码
> window.hide()
...
-<font color='red'>一句话总结回调：</font>

> 是否需要知道调用 **精确完成时间**

回调的命名：

> 回调，即down -------> 命名加“ed”和 “on”：**paused** : 应用进入
...
> 1、**原则：**生效地方，只能有一个！！！！！<font color='red'>只能由一个变量承载！</font>！！！<font color='red'>这样才概念清晰</font>
>
> > ~~配置可以有多个通路/变量~~
>
> 
